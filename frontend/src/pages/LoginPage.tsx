    ...
    <div style={{ minHeight: "100vh", background: PALETTE.cream, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 440, background: PALETTE.panel, borderRadius: 28, padding: "48px 40px", boxShadow: `0 8px 0 ${PALETTE.deepCream}` }}>
        ...
        <p style={{ fontSize: 14, color: PALETTE.ink, opacity: 0.6, marginBottom: 32 }}>
          No account yet?{" "}
          <Link to="/register" style={{ color: PALETTE.accent, fontWeight: 600, textDecoration: "none" }}>Sign up</Link>
        </p>

        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: PALETTE.ink, marginBottom: 6 }}>Email</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: PALETTE.ink, marginBottom: 6 }}>Password</label>
            <input
              type="password"
              value={password}
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ marginTop: 8, background: loading ? PALETTE.deepCream : PALETTE.accent, color: loading ? PALETTE.ink : "#fff", border: "none", borderRadius: 14, padding: "16px", fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : `0 5px 0 ${PALETTE.accentDeep}`, letterSpacing: 0.2 }}
          >
